...
public abstract class ChessGame {
    ...
    public void undoMove() {
        if (!stack.isEmpty()) {
            ...
            backendRestore(memento.getBoard());
        }
    }

    public void backendRestore(ChessBoard b) {
        for (int i = 0; i < 8; i++) {
            for (int j = 0; j < 8; j++) {
                Square s = new Square(BoardFile.values()[j], BoardRank.values()[i]);
                Piece p = b.getPieceAtSquare(s);
                getBoard().setPieceAtSquare(s, p);
            }
        }
        if (getWhoseTurn() == WHITE) {
    ...
    private void storeBoard() {
        stack.push(new Memento(getBoard()));
    }
